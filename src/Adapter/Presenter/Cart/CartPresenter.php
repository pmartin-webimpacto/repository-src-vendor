...
class CartPresenter implements PresenterInterface
{
    ...
    private function getTemplateVarVouchers(Cart $cart)
    {
        ...
        foreach ($cartVouchers as $cartVoucher) {
            ...
            if (isset($cartVoucher['free_shipping']) && $cartVoucher['free_shipping'] == '1' && isset($cartVoucher['value_real']) && $cartVoucher['reduction_amount'] == '0.00' && $cartVoucher['reduction_percent'] == '0.00') {
                $cartVoucher['reduction_formatted'] = $cartVoucher['value_real'] . '€';
            } elseif (isset($cartVoucher['free_shipping']) && $cartVoucher['free_shipping'] == '1' && isset($cartVoucher['value_real']) && isset($cartVoucher['reduction_percent']) && $cartVoucher['reduction_amount'] == '0.00') {
                $cartVoucher['reduction_formatted'] = $cartVoucher['value_real'] . '€';
            } elseif (isset($cartVoucher['free_shipping']) && $cartVoucher['free_shipping'] == '1' && isset($cartVoucher['value_real']) && isset($cartVoucher['reduction_amount']) && $cartVoucher['reduction_amount'] > 0) {
                $cartVoucher['reduction_formatted'] = $cartVoucher['value_real'] . '€';
            ...
            $prefijo = explode("_",$cartVoucher['obj']->code);
            $vouchers[$cartVoucher['id_cart_rule']]['prefijo'] = $prefijo[0];

            if (CartRule::checkCartRulePromo((int)$vouchers[$cartVoucher['id_cart_rule']]['id_cart_rule'])){
                $isPromo = 1;
            }else{
                $isPromo = 0;
            }
